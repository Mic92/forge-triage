...
User-defined command palette for PR notifications. Allows users to configure arbitrary named commands in `commands.toml` that are invoked against the currently focused PR with template variable substitution.
...
variables are `{pr_number}`, `{branch}`, `{repo}`, `{repo_owner}`, and `{repo_name}`.
`{repo}`, `{repo_owner}`, and `{repo_name}` SHALL always be available. `{pr_number}` and
`{branch}` SHALL only be available after PR details have been fetched. If a command arg
references a variable that is unavailable or unknown, the system SHALL display an error
notification and SHALL NOT execute the command.
...
- **WHEN** a user command using only `{repo}`, `{repo_owner}`, or `{repo_name}` is selected
- **THEN** the system SHALL substitute those variables and execute the command even if PR details are not yet loaded
...
### Requirement: Optional working directory and environment variables
The system SHALL support optional `cwd` and `env` fields on each command. `cwd` specifies
a working directory for the subprocess (supporting template vars and `$HOME`/`~` expansion).
`env` specifies a dict of environment variable overrides (values support template substitution)
merged on top of the current process environment.

#### Scenario: Running a command with cwd
- **WHEN** a command has a `cwd` field set
- **THEN** the system SHALL run the subprocess with the resolved working directory

#### Scenario: Running a command with env overrides
- **WHEN** a command has an `env` field set
- **THEN** the system SHALL run the subprocess with the current environment plus the specified overrides, with template variables substituted in values
